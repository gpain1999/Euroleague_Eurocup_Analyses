...
for index, row in data.iterrows():
    ...
    if f"{team}_{id_players}" in png_files :
        ...
        player_image = player_image.resize((700,933), Image.Resampling.LANCZOS)
        ...
        panini_image.paste(player_image_with_bg, (0, 150))
        ...
        panini_image.paste(team_image, (width - 200, 40), mask=team_image)


        #COMPETITON
        competition_image_path = f"images/{competition}.png"
        competition_image = Image.open(competition_image_path)
        new_size = (int(competition_image.width*0.75), int(competition_image.height*0.75))
        ...
        panini_image.paste(competition_image, (40, 40), mask=competition_image)
        ...
        # text_width = bbox[2] - bbox[0]  # Largeur = x_max - x_min
        # text_height = bbox[3] - bbox[1]  # Hauteur = y_max - y_min
        # draw.text((0, 50), f"{season}-{season+1}", fill=text_color, font=ImageFont.truetype(font_path_bold, 35))
