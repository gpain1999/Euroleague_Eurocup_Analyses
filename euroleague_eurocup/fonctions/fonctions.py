...
def recuperation_pbp(competition,season,data_dir) :
    ...
    _pbp.to_csv(os.path.join(data_dir, f'{competition}_pbp_{season}.csv'),index = False)

def recuperation_gs(competition,season,data_dir) : 
    if competition == "eurocup" :
        gs_ = GameStats(competition="U")
    else :
        gs_ = GameStats(competition="E")

    #euroleague_gs = gs_euroleague.get_game_reports_single_season(season=season)
    _gs = gs_.get_game_reports_single_season(season=season)

    #euroleague_gs.to_csv(os.path.join(data_dir, f'euroleague_gs_{season}.csv'),index = False)
    _gs.to_csv(os.path.join(data_dir, f'{competition}_gs_{season}.csv'),index = False)
