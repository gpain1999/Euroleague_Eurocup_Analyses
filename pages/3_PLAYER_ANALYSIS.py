...
with col1:
    ...
    with cola : 
        ...
        fig2 = f.plot_semi_circular_chart(df_resultat["1_R"].sum()/df_resultat["1_T"].sum() if df_resultat["1_T"].sum() != 0 else 0,"1P",size=int(100*zoom),font_size=16,m=False)
        st.plotly_chart(fig2)
        fig2 = f.plot_semi_circular_chart(df_resultat["2_R"].sum()/df_resultat["2_T"].sum() if df_resultat["2_T"].sum() != 0 else 0,"2P",size=int(100*zoom),font_size=16,m=False)
        st.plotly_chart(fig2)
        fig2 = f.plot_semi_circular_chart(df_resultat["3_R"].sum()/df_resultat["3_T"].sum() if df_resultat["3_T"].sum() != 0 else 0,"3P",size=int(100*zoom),font_size=16,m=False)
        st.plotly_chart(fig2)
